...
export function LegalPageGeneratorPage() {
  ...
  const [pageType, setPageType] = useState(() => loadFromLocalStorage('pageType', 'terms'))
  const [formData, setFormData] = useState<FormData>(() => loadFromLocalStorage('formData', defaultFormData))
  const [generatedContent, setGeneratedContent] = useState('')
  const [showRawHTML, setShowRawHTML] = useState(() => loadFromLocalStorage('showRawHTML', false))

  // Save to localStorage whenever state changes
  useEffect(() => {
    saveToLocalStorage('pageType', pageType)
  }, [pageType, saveToLocalStorage])
  ...
  useEffect(() => {
    saveToLocalStorage('showRawHTML', showRawHTML)
  }, [showRawHTML, saveToLocalStorage])

  // Check if there's data to reset
  const hasDataToReset = useMemo(() => {
    return Object.entries(formData).some(([key, value]) => {
      const defaultValue = defaultFormData[key as keyof FormData]
      return value !== defaultValue
    }) || pageType !== 'terms' || showRawHTML !== false
  }, [formData, pageType, showRawHTML, defaultFormData])

  // Check if all required fields are populated
  const isFormValid = useMemo(() => {
    const requiredFields = ['companyName', 'address1', 'city', 'state', 'postalCode', 'country', 'websiteUrl', 'contactUrl']
  ...
  }, [clearLocalStorage, defaultFormData])
...
<p>We use both Session and Persistent Cookies for purposes including:</p>
...
<p>The Company will retain Your Personal Data as long as necessary to fulfill the purposes outlined in this Privacy Policy. Specific retention periods will be provided upon request or specified in applicable legal obligations.</p>
<p>---</p>
<h2>Sale of Personal Data</h2>
<p>The Company does not sell, rent, or trade Your Personal Data to third parties.</p>
<p>---</p>
<h2>International Transfers and Legal Basis</h2>
<p>If Your data is transferred internationally, We ensure compliance with applicable laws, including GDPR provisions, through Standard Contractual Clauses or similar safeguards.</p>
<p>---</p>
...
<p>For further inquiries, <a href="${data.contactUrl}">contact us</a>.</p>
<p></p>
<p></p>
<p><br></p>`
        break
      ...
      case 'goodfaith':
        content = `<h2>YOU HAVE THE RIGHT TO RECEIVE A "GOOD FAITH ESTIMATE" EXPLAINING HOW MUCH YOUR MEDICAL CARE WILL COST</h2>
...
<ul>
<li><p>You have the right to receive a Good Faith Estimate for the total expected cost of any non-emergency items or services. This includes related costs like medical tests, prescription drugs, equipment, and hospital fees.</p></li>
<li><p>Make sure your health care provider gives you a Good Faith Estimate in writing at least 1 business day before your medical service or item. You can also ask your health care provider, and any other provider you choose, for a Good Faith Estimate before you schedule an item or service.</p></li>
<li><p>If you receive a bill that is at least $400 more than your Good Faith Estimate, you can dispute the bill.</p></li>
<li><p>Make sure to save a copy or picture of your Good Faith Estimate.</p></li>
</ul>
<p>For questions or more information about your right to a Good Faith Estimate, visit <a href="http://www.cms.gov/nosurprises" target="_blank">http://www.cms.gov/nosurprises</a> or call 800-985-3059.</p>`
        break
    }
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This will clear all form data including company information, addresses, and URLs. 
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            {showRawHTML ? (
              ...
              <div
                id="legal-output"
                className="max-w-none bg-muted p-6 rounded-lg max-h-96 overflow-y-auto legal-content"
